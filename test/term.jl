...
@testset "basics" begin
	...
	@testset "moments" begin
		...
		@test MSOS.clear_zeros([0x0002, 0x0000, 0x0001]) == [0x0001, 0x0002]
...
@testset "sparsity" begin
	...
	@testset "term" begin
		...
		variable_sets = [[1, 2, 3, 4]]
		monomial_sets = [[[0x0001], [0x0002, 0x0003], [0x0002, 0x0004]]]

		blocks = MSOS.compute_tsp_blocks_X_0(variable_sets[1], monomial_sets[1], 1)

		@test blocks == [[1, 2], [3, 4, 5]]

		blocks = MSOS.compute_tsp_blocks_X_0(variable_sets[1], monomial_sets[1], 2)

		@test blocks == [[1, 2, 6, 10, 11, 12, 13, 15], [3, 4, 5], [7], [8], [9], [14]]

		@test [MSOS.clear_zeros(alpha[2]) for (j, alpha) in MSOS.moment_columns(blocks[2], [1, 2, 3, 4], 2)] == [[0x0002], [0x0003], [0x0004]]
		@test [MSOS.clear_zeros(alpha[2]) for (j, alpha) in MSOS.moment_rows(blocks[2], [1, 2, 3, 4], 2, 2)] == [[0x0002], [0x0003]]

		@test [block for (_, block) in MSOS.matrix_blocks(blocks)] == [[1, 2, 6, 10, 11, 12, 13, 15], [3, 4, 5]]
		@test MSOS.diagonal_block(blocks) == [7, 8, 9, 14]
